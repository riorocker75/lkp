          ...
          <div class="row">
            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                <form action="{{ url('/dashboard/jadwal/update') }}" method="post">
                       @csrf  
                       @method('POST')
                    <div class="form-group">
                        <label>Nama Jadwal</label>  
                        <input type="hidden" name="id" value="{{$dj->id}}">  
                        <input type="text" class="form-control" name="nama" value="{{$dj->nama}}" required>
                    </div>
                     ...
                     <div class="form-group">
                            ...
                            <select class="form-control select2" style="width: 100%;" name="pelatihan_id" required>
                                @php
                                    ...
                                    $latih_dj= App\Models\Pelatihan::where('id',$dj->pelatihan_id)->first();
                                 
                                @endphp
                                    <option selected value="{{$latih_dj->id}}" required>{{$latih_dj->nama}}</option>
                    ...
                    <div class="form-group">
                        <label>Kuota</label>    
                        <input type="number" class="form-control" name="kuota" value="{{$dj->kuota}}" required>
                    </div>
                      <div class="form-group">
                            <label>Jadwal Mulai</label>    
                            <input type="date" class="form-control" name="jadwal_awal" value="{{date('Y-m-d', strtotime($dj->jadwal_awal))}}" required>
                    </div>

                    <div class="form-group">
                        <label>Jadwal Akhir</label>    
                        <input type="date" class="form-control" name="jadwal_akhir" value="{{date('Y-m-d', strtotime($dj->jadwal_akhir))}}" required>
                    ...
                    <div class="form-group">
                            ...
                            <select class="form-control select2" style="width: 100%;" name="status" required>
                                <option selected value="{{$dj->status}}">{{stat_latih($dj->status)}}</option>
